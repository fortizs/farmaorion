...
{
	public partial class ProductoMoto : PageBase
	{
		#region Inicio
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			if (!Page.IsPostBack)
			{
				...
				CargarDDL(ddlIDTipoCombustible, new BLTipoCombustible().TipoCombustibleListar(IDEmpresa()), "IDTipoCombustible", "Nombre", true, Constantes.SELECCIONAR);
				CargarDDL(ddlIDColor, new BLColor().ColorListar(IDEmpresa()), "IDColor", "Nombre", true, Constantes.SELECCIONAR);
				CargarDDL(ddlIDCarroceria, new BLCarroceria().CarroceriaListar(IDEmpresa()), "IDCarroceria", "Nombre", true, Constantes.SELECCIONAR);

				

				CargaAnioModelo(1970);
				MarcaListar();
				ModeloListar();
				ModeloVersionListar();
		...
		{
			CargarDDL(ddlIDMarcaVehiculo, new BLMarcaModeloVersion().MarcaListar("0", 0), "IDMarca", "Marca", true, Constantes.NINGUNO);
		}

		private void ModeloListar()
		{
			CargarDDL(ddlIDModelo, new BLMarcaModeloVersion().ModeloListar("0", Int32.Parse(ddlIDMarcaVehiculo.SelectedValue), 0), "IDModelo", "Modelo", true, Constantes.NINGUNO);
		}

		private void ModeloVersionListar()
		{
			CargarDDL(ddlIDModeloVersion, new BLMarcaModeloVersion().ModeloVersionListar("0", Int32.Parse(ddlIDMarcaVehiculo.SelectedValue), Int32.Parse(ddlIDModelo.SelectedValue), 0), "IDModeloVersion", "ModeloVersion", true, Constantes.NINGUNO);
		}


		protected void ddlIDMarcaVehiculo_SelectedIndexChanged(object sender, EventArgs e)
		{
			ModeloListar();
		}

		protected void ddlIDModelo_SelectedIndexChanged(object sender, EventArgs e)
		{
			ModeloVersionListar();
		}

		private void CargaAnioModelo(Int32 pAnioInicial)
		{
			Int32 AnioActual = DateTime.Now.Year;
			for (int i = AnioActual + 1; i > pAnioInicial; i--)
			{ 
				ddlAnioModelo.Items.Add(new ListItem(i.ToString(), i.ToString()));

			} 
			ddlAnioModelo.SelectedValue = DateTime.Today.Year.ToString();
		}
		...
		protected void ProductoSeleccionar()
		{
			...
			txtChasis.Text = oBE.Chasis;
			txtNumeroLote.Text = oBE.NumeroLote;
			txtMotor.Text = oBE.Motor;
			ddlIDTipoCombustible.SelectedValue = oBE.IDTipoCombustible.ToString();
			ddlIDColor.SelectedValue = oBE.IDColor.ToString();
			txtNumeroDUA.Text = oBE.NumeroDUA;
			txtFechaDUA.Text = oBE.FechaDUA.ToShortDateString();
			ddlAnioModelo.SelectedValue = oBE.AnioModelo.ToString();
			ddlIDCarroceria.SelectedValue = oBE.IDCarroceria.ToString();

			ddlIDMarcaVehiculo.SelectedValue = oBE.IDMarcaVehiculo.ToString();
			ddlIDModelo.SelectedValue = oBE.IDModelo.ToString();
			ddlIDModeloVersion.SelectedValue = oBE.IDModeloVersion.ToString();
		...
		protected void btnGuardar_Click(object sender, EventArgs e)
		{
			...
			if (ddlIDMarcaVehiculo.SelectedValue == "0") validacion.Append("<div>Seleccione Marca Vehículo</div>");
			if (ddlIDModelo.SelectedValue == "0") validacion.Append("<div>Seleccione Modelo</div>");
			if (ddlIDModeloVersion.SelectedValue == "0") validacion.Append("<div>Seleccione Versión</div>");
			if (txtNombre.Text.Trim().Length == 0) validacion.Append("<div>Ingrese Descripción</div>");

			if (txtChasis.Text.Trim().Length == 0) validacion.Append("<div>Ingrese Chasis</div>");
			if (txtNumeroLote.Text.Trim().Length == 0) validacion.Append("<div>Ingrese Lote</div>");
			if (txtMotor.Text.Trim().Length == 0) validacion.Append("<div>Ingrese Nro.Motor</div>"); 
			if (txtNumeroDUA.Text.Trim().Length == 0) validacion.Append("<div>Ingrese Nro.DUA</div>");

			if (ddlIDCarroceria.SelectedValue == "0") validacion.Append("<div>Seleccione Carroceria</div>");
			...
			oBE.Chasis = txtChasis.Text.Trim();
			oBE.NumeroLote = txtNumeroLote.Text.Trim();
			oBE.Motor = txtMotor.Text.Trim();
			oBE.IDTipoCombustible = Int32.Parse(ddlIDTipoCombustible.SelectedValue);
			oBE.IDColor = Int32.Parse(ddlIDColor.SelectedValue);
			oBE.NumeroDUA = txtNumeroDUA.Text.Trim();
			oBE.FechaDUA = DateTime.Parse(txtFechaDUA.Text.Trim());
			oBE.AnioModelo = Int32.Parse(ddlAnioModelo.SelectedValue);
			oBE.IDModeloVersion = Int32.Parse(ddlIDModeloVersion.SelectedValue);
			oBE.IDCarroceria = Int32.Parse(ddlIDCarroceria.SelectedValue);
		...
		private void LimpiarFormulario()
		{
			...
			ddlIDTipoCombustible.SelectedIndex = -1;
			ddlIDColor.SelectedIndex = -1;
			txtNumeroDUA.Text = String.Empty;
			txtFechaDUA.Text = String.Empty;
			ddlAnioModelo.SelectedIndex = -1;
			ddlIDMarcaVehiculo.SelectedIndex = -1;
			ddlIDModelo.SelectedIndex = -1;
			ddlIDModeloVersion.SelectedIndex = -1;
			ddlIDCarroceria.SelectedIndex = -1;
